...
#include "tpm_i2c_infineon.h"
#include <opal-api.h>

//#define DBG(fmt, ...) prlog(PR_ERR, fmt, ##__VA_ARGS__)
#define DBG(fmt, ...)

#define DRIVER_NAME "i2c_tpm_infineon"
...
#define TPM_STS			0x01
#define TPM_BURST_COUNT		0x02
#define TPM_DATA_FIFO_W		0x05
#define TPM_DATA_FIFO_R		0x05
...
static int tpm_wait_for_command_ready(void)
{
	...
	do {
		...
		if (rc < 0) {
			 ...
			 */
			prlog(PR_ERR, "INFINEON: fail to read sts.commandReady, "
			      "rc=%d\n", rc);
		...
		if (tpm_check_status(status,
				     ...
				     TPM_STS_COMMAND_READY)) {
			DBG("--- Command ready, status=%#x, delay=%lu/%d\n",
			    status, tb_to_msecs(now-start), TPM_TIMEOUT_B);
			return 0;
		}
...
static int tpm_set_command_ready(void)
{
	...
	for (retries = 0; retries < 2; retries++) {
		...
		if (rc < 0) {
			 ...
			 * the return code (rc) for further details.
			 */
			prlog(PR_ERR, "INFINEON: fail to write sts.commandReady, "
			      "rc=%d\n", rc);
			return STB_DRIVER_ERROR;
		}
	 ...
	 */
	prlog(PR_ERR, "INFINEON: timeout on sts.commandReady, delay > %d\n",
	      2*TPM_TIMEOUT_B);
	return STB_TPM_TIMEOUT;
}
...
static int tpm_wait_for_fifo_status(uint8_t mask, uint8_t expected)
{
	...
	for(retries = 0; retries <= MAX_STSVALID_POLLS; retries++) {
		...
		if (rc < 0) {
			 ...
			 * the return code (rc) for further details.
			 */
			prlog(PR_ERR, "INFINEON: fail to read fifo status: "
...
static int tpm_wait_for_data_avail(void)
{
	...
	do {
		...
		if (rc < 0) {
			 ...
			 * the return code (rc) for further details.
			 */
			prlog(PR_ERR, "INFINEON: fail to read sts.dataAvail, "
			      "rc=%d\n", rc);
			return STB_DRIVER_ERROR;
		}
		if (tpm_check_status(status,
	 ...
	 */
	prlog(PR_ERR, "INFINEON: timeout on sts.dataAvail, delay=%lu/%d\n",
	      tb_to_msecs(now-start), TPM_TIMEOUT_A);
	return STB_TPM_TIMEOUT;
}
...
static int tpm_read_burst_count(void)
{
	...
	do {
		now = mftb();
		rc = tpm_i2c_request_send(tpm_device, SMBUS_READ, TPM_BURST_COUNT, 1,
					  buf, sizeof(buf));
		if (rc < 0) {
			/**
			 ...
			 * the return code (rc) for further details.
			 */
			prlog(PR_ERR, "INFINEON: fail to read sts.burstCount, "
			      "rc=%d\n", rc);
			return STB_DRIVER_ERROR;
		}

		burst_count = (buf[2] << 16) + (buf[1] << 8) + buf[0];

		if (burst_count > 0 && burst_count != 0xffffff) {
	 ...
	 * increased or the TPM device is not functional.
	 */
	prlog(PR_ERR, "INFINEON: timeout on sts.burstCount, delay=%lu/%d\n",
	      tb_to_msecs(now-start), TPM_TIMEOUT_D);
	return STB_TPM_TIMEOUT;
}
...
static int tpm_write_fifo(uint8_t* buf, size_t buflen)
{
	...
	do {
		...
		if (rc < 0) {
			 ...
			 * the return code (rc) for further details.
			 */
			prlog(PR_ERR, "INFINEON: fail to write fifo, "
			      "count=%zd, rc=%d\n", count, rc);
			return STB_DRIVER_ERROR;
		}
		...
		if (rc == STB_TPM_TIMEOUT) {
			 ...
			 */
			prlog(PR_ERR, "INFINEON: write FIFO overflow, not expecting "
			      "more data\n");
	...
	if (rc < 0) {
		 ...
		 */
		prlog(PR_ERR, "INFINEON: fail to write fifo (last byte), "
		      "count=%zd, rc=%d\n", count, rc);
		return STB_DRIVER_ERROR;
	}
	rc = tpm_wait_for_fifo_status(TPM_STS_VALID | TPM_STS_EXPECT,
...
static int tpm_read_fifo(uint8_t* buf, size_t* buflen)
{
	...
	do {
		...
		if (count + burst_count > *buflen) {
			 ...
			 */
			prlog(PR_ERR, "INFINEON: overflow on fifo read, c=%zd, "
		...
		if (rc < 0) {
			 ...
			 * (rc) for further details.
			 */
			prlog(PR_ERR, "INFINEON: fail to read fifo, count=%zd, "
			      "rc=%d\n", count, rc);
			rc = STB_DRIVER_ERROR;
			goto error;
		}
			...
			size_t* buflen)
{
	...
	if (rc < 0) {
		 ...
		 * (rc) for further details.
		 */
		prlog(PR_ERR, "INFINEON: fail to write sts.go, rc=%d\n", rc);
		rc = STB_DRIVER_ERROR;
		goto out;
	}
	...
	if (rc < 0) {
		 ...
		 * (rc) for further details.
		 */
		prlog(PR_ERR, "INFINEON: fail to release tpm, rc=%d\n", rc);
		rc = STB_DRIVER_ERROR;
	}
...
static struct tpm_driver tpm_i2c_infineon_driver = {
	.name     = DRIVER_NAME,
	.transmit = tpm_transmit,
};

void tpm_i2c_infineon_probe(void)
{
	struct tpm_dev *tpm_device = NULL;
	struct dt_node *node = NULL;

	dt_for_each_compatible(dt_root, node, "infineon,slb9645tt") {
		if (!dt_node_is_enabled(node))
			continue;

		tpm_device = (struct tpm_dev*) malloc(sizeof(struct tpm_dev));
		assert(tpm_device);
		tpm_device->tpm_version = 0x0102;
		...
		if (!tpm_device->i2c_addr && !dt_find_property(node, "reg")) {
			prlog(PR_ERR, "INFINEON: reg property not found, "
			      "tpm node %p\n", node);
			goto disable;
		}
		...
		if (!tpm_device->bus_id &&
			...
			prlog(PR_ERR, "INFINEON: ibm,opal-id property not "
			      "found, tpm node parent %p\n", node->parent);
			goto disable;
		}

		if (tpm_register_chip(node, tpm_device,
				      &tpm_i2c_infineon_driver)) {
			free(tpm_device);
			continue;
		}
	}
